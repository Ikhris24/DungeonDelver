...
public class Grappler : MonoBehaviour, IGadget
{
    ...
    private void OnTriggerEnter2D(Collider2D colliga)
    {
        string otherLayer =LayerMask.LayerToName(colliga.gameObject.layer);

        switch (otherLayer) 
        {
            case "Items":
                PickUp pup = colliga.GetComponent<PickUp>();
                if (pup == null) return;
                ...
                break;
            case "Enemies":
                Enemy e = colliga.GetComponent<Enemy>();
